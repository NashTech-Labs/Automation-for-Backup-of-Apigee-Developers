...
pipeline {
    ...
    stages {
        ...
        stage("Exporting developers from Apigee") {
            steps {
            script {
            def currentDate = new Date().format("dd-MM-yyyy")
                    def exportDir = "<APIGEE_ORG_NAME>/${currentDate}/developers"
                    
                    sh """
                    mkdir -p ${exportDir}
                    ${APIGEE_CLI_DIR}/apigeecli developers export -o <APIGEE_ORG_NAME> -t ${TOKEN} 
                    """
                    sh "mv developers.json ${exportDir}"
            ...
            if (changes) {
                sh "git commit -m 'Added developers from Jenkins Pipeline'"
                withCredentials([gitUsernamePassword(credentialsId: "${GITHUB_CREDS}", gitToolName: 'Default')]) {
                    sh "git push -u origin main"
                }
            } else {
                echo "No changes to commit in developers. Skipping git push."
                }
            }
        }
    }
